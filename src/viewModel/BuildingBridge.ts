...
export class BuildingBridge {
  protected static readonly FLOOR_PX_HEIGHT =
    buildingViewConstants.FLOOR_PX_HEIGHT;
  protected static readonly ESTIMATE_TIME_UPDATE_THRESHOLD =
    buildingViewConstants.ESTIMATE_TIME_UPDATE_THRESHOLD;
  ...
  protected static playDingSound = () => {
    ...
    BuildingBridge.currentSoundIndex =
      (BuildingBridge.currentSoundIndex + 1) % BuildingBridge.dingSounds.length;

    // Clone and play to handle overlapping sounds - with proper type casting
    const clonedAudio = audio.cloneNode(true) as HTMLAudioElement;
    clonedAudio.play().catch((e) => console.error('error in play ding:', e));
  };
  ...
  private handleElevatorMove(elevatorId: number, logicalPosition: number) {
    const pixelPosition =
      BuildingBridge.convertLogicalToVisual(logicalPosition);
    console.log(
      `🔥 HandleElevatorMove - Elevator ${elevatorId}, Logical: ${logicalPosition}, Pixel: ${pixelPosition}`
    );
    ...
    this.setStoreState((state: any) => {
      ...
      const elevatorIndex = building.elevators.findIndex(
        (e: any) => e.id === elevatorId
      );
  ...
  private handleElevatorAdded() {
    ...
    const pxPos = BuildingBridge.convertLogicalToVisual(
    ...
    this.setStoreState((state: any) => {
      ...
      return {
        buildings: {
          ...
          [this.buildingId]: {
            ...building,
            elevators: building.elevators.filter(
              (e: any) => e.id !== elevatorId
            ),
          },
        },
  ...
  private handleElevatorEstimateTimeUpdate(elevatorId: number) {
    console.log(
      `[BuildingBridge ${this.buildingId}] Handling estimate time update for elevator ${elevatorId}`
    );
        ...
        buildings: {
          ...
          [this.buildingId]: {
            ...
            elevators: modelElevators.map((elevator) => ({
              id: elevator.id,
              pixelPosition: BuildingBridge.convertLogicalToVisual(
  ...
  // BuildingBridge.ts - תיקון המתודה startEstimateTimeUpdater
    ...
    this.estimateTimeInterval = setInterval(() => {
      ...
      this.setStoreState((state: any) => {
        ...
        const updatedFloors = building.floors.map((floor: any) => {
          ...
          if (!modelFloor) return floor;

          let updatedFloor = { ...floor };

          // Update estimate time
          if (!modelFloor.isCallingToElevator) {
            if (floor.estimateTime > 0) {
              hasChanges = true;
              updatedFloor.estimateTime = 0;
            }
          } else {
            const newEstimateTime = Math.max(
              0,
              modelFloor.estimatedWaitTimeSeconds
            );
            // Only update if significant change (more than threshold)
            if (
              Math.abs(floor.estimateTime - newEstimateTime) >
              BuildingBridge.ESTIMATE_TIME_UPDATE_THRESHOLD
            ) {
              hasChanges = true;
              updatedFloor.estimateTime = newEstimateTime;
            }
          }

          // Update boarding state
          if (floor.boardingState !== modelFloor.elevatorBoardingState) {
            hasChanges = true;
            updatedFloor.boardingState = modelFloor.elevatorBoardingState;
          }

          if (
            Math.abs(
              floor.boardingTimeRemaining - modelFloor.boardingTimeRemaining
            ) > 0.01
          ) {
            hasChanges = true;
            updatedFloor.boardingTimeRemaining =
              modelFloor.boardingTimeRemaining;
          }
